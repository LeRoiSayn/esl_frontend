...
const GREET = {
  ...
  en: {
    ...
    admin: (n, t) => ({
      message: `${t} ${n}! 👋\n\nI'm **Simon**, your  assistant. As administrator you have access to all system data.\n\n💡 **Questions you can ask:**\n• "Search for student Smith"\n• "Global statistics"\n• "Institutional KPIs"\n• "Student alerts"\n• "Grades submitted by teachers"\n• "Monthly financial report"`,
      quickActions: [
...
const Chatbot = () => {
  ...
  const checkNewStudentContent = async (lang) => {
    try {
      ...
      const newItems = (res.data.notifications || []).filter((n) =>
        contentTypes.includes(n.type),
      );
  ...
  const addBotMessage = (text, quickActions = null, data = null) => {
    setMessages((prev) => [
      ...prev,
      { sender: "bot", text, quickActions, data, timestamp: new Date() },
    ]);
  };
      ...
      <AnimatePresence>
        {isOpen && (
          ...
          >
            {/* ── Header ── */}
            <div
              className={`flex items-center justify-between p-4 bg-gradient-to-r ${getRoleColor()} text-white`}
            >
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs opacity-80">• {uiL.online}</p>
                </div>
              </div>

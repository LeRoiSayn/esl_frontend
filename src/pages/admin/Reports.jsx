        ...
        const sections = Object.entries(byYear)
          ...
          .map(([yr, list]) => {
            ...
            <table><thead><tr>
              ...
              <th>Professeur</th><th>Salle</th><th style="text-align:center">Capacité</th><th>Départements</th>
  ...
  const handleTeachers = async () => {
    ...
    try {
      await openReportAsyncSafe(t("admin_report_teachers_title"), async () => {
        ...
        const sections = Object.entries(byFaculty)
          .sort(([a], [b]) => a.localeCompare(b))
          .map(([fac, depts]) => {
            const facTotal = Object.values(depts).reduce(
              (s, a) => s + a.length,
              0,
            );
            const deptSections = Object.entries(depts)
              .sort(([a], [b]) => a.localeCompare(b))
              .map(([dept, list]) => {
                const rows = list
                  .map((t) => {
  ...
  const handleCourses = async () => {
    ...
    try {
      await openReportAsyncSafe(t("admin_report_courses_title"), async () => {
        ...
        const courses =
          coursesRes.data?.data?.data || coursesRes.data?.data || [];
        ...
        const sections = Object.entries(byDept)
          ...
          .map(([dept, list]) => {
            const sorted = [...list].sort((a, b) => {
              const ia = LEVEL_ORDER_MAP[a.level] ?? 99,
                ib = LEVEL_ORDER_MAP[b.level] ?? 99;
    ...
    datasets: [
      {
        ...
        backgroundColor: generateColors(
          stats?.students_by_department?.length || 0,
        ),
        borderWidth: 0,
      },
    ],
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        <StatCard
          title={t("reports_total_students")}
          value={stats?.stats?.total_students || 0}
          icon={UserGroupIcon}
          color="primary"
        />
        <StatCard
          title={t("reports_active_students")}
        ...
        />
        <StatCard
          title={t("reports_total_teachers")}
        ...
        />
        <StatCard
          title={t("reports_active_teachers")}
      ...
      <div>
        ...
        >
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">
            Générez des rapports détaillés prêts à imprimer ou à exporter
          </p>
        </motion.div>

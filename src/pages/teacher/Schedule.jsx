  ...
  return (
    <div className="space-y-6">
      <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }}>
        ...
        <p className="text-gray-500 dark:text-gray-400">{t('schedule_subtitle_teacher')}</p>
      </motion.div>
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {DAYS.map((day) => (
          <div key={day} className="card p-6">
            <h3 className="font-semibold text-lg text-gray-900 dark:text-white mb-4 capitalize">{t(day)}</h3>
